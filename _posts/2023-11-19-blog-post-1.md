...
$$a_n =\frac{2}{L}\int_0^L \frac{\rho g}{Y}\left(Lx -\frac{1}{2}x^2 \right)\cos(\frac{n\pi}{L}x) \d x$$

帶入積分公式：
\begin{align}
    \int_0^\pi \theta \cos(n\theta) \d \theta &=-\frac{2}{n^2}\;\text{($n$ odd)}/=0\;\text{($n$ even)} \\
    \int_0^\pi \theta^2 \cos(n\theta) \d \theta &=-\frac{2\pi}{n^2}\;\text{($n$ odd)}/=\frac{2\pi}{n^2}\;\text{($n$ even)} 
\end{align}
